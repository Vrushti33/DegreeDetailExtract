...
_FONT_FILES: Dict[str, List[str]] = {
    ...
    "script": [
        # Great Vibes: flowing script — used for student name on real certs
        "GreatVibes-Regular.ttf",
        # Fallbacks if not downloaded
...
def get_fonts(scale: float = 1.0) -> Dict[str, ImageFont.ImageFont]:
    ...
    Keys (new — calligraphy / formal)
    ----------------------------------
    script       — Great Vibes flowing script (student name)
    ...
    return {
        ...
        # ── Calligraphy / formal slots (new) ───────────────────────────────────
        # student name in flowing script — mimics real handwritten-style certs
        "script":       _load("script",          int(base * 2.00)),
        "script_sm":    _load("script",          int(base * 1.40)),
